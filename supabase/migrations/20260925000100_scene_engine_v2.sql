-- Motor criativo v2 (cenas) — docs/motor-v2.
--
-- 1) presentations.engine_version: 1 = motor atual, 2 = motor de cenas.
--    Decks antigos ficam com 1 e renderizam exatamente como antes. O slide
--    também carrega content.engine_version, então a coluna serve para
--    comparação de custo/qualidade no painel (generation_logs.metadata tem o
--    mesmo dado) e para consultas.
--
-- 2) presentation_asset_quotas: cota de ativos emitida pela geração, atrelada
--    à apresentação e ao número de imagens planejadas. fetch-image a consome no
--    lugar do limite genérico por hora (que continua valendo para chamadas
--    avulsas do Editor). Permite que a resolução progressiva das imagens seja
--    retomada numa próxima abertura do Editor sem esbarrar no teto de 15/h.
--
-- NÃO toca na tabela de créditos nem nas funções de cobrança/estorno.
...
-- Leitura: o dono (progresso dos ativos) e a equipe (painel de métricas).
-- Escrita: somente service_role (generate-presentation e fetch-image).
...
-- Consome 1 unidade da cota ("ai" ou "photo"). Atômico: a condição de saldo
-- está no próprio UPDATE, então duas chamadas concorrentes nunca passam do
-- planejado. Só o dono da apresentação consome.
...
-- Soma o custo real (usage da API ou tabela por imagem) das imagens de IA
-- geradas para a apresentação. Teto por chamada evita valores absurdos.
